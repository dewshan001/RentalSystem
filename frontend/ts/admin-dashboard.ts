...
/* â”€â”€â”€ State â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
    ...
    function navigate(el) {
      ...
      if (view === 'users') loadUserList();
      if (view === 'payments') initPayments();
    ...
    function applyTheme(theme) {
      ...
      syncChartDefaults();
      if (barChartInst || pieChartInst) buildCharts();
    }
    ...
    window.addEventListener('load', () => {
      ...
      if (!staffId || staffRole !== 'admin') { window.location.href = '/staff-login'; return; }
    ...
    function initPayments() {
      ...
      if (!paymentData.length) loadPayments();
    }
    ...
    function onPaymentModalBgClick(e) {
      if (e.target && e.target.id === 'recordPaymentModal') closePaymentModal();
    }
    ...
    async function searchInvoices() {
      ...
      try {
        ...
        if (hint) hint.textContent = invoiceMatches.length + ' invoice(s) found';
      } catch (err) {
    ...
    function onInvoiceSelected() {
      ...
      if (hint) hint.textContent = `Selected: ${selectedInvoice.invoiceNumber || ''} | Balance Due: Rs. ${bal.toFixed(2)}`;
    }
    ...
    function renderPaymentTable(list = paymentData) {
      ...
      if (count) count.textContent = `${list.length} payment${list.length !== 1 ? 's' : ''}`;
      if (!tbody) return;
      if (!list.length) {
    ...
    async function deletePayment(id, paymentNumber) {
      const msg = document.getElementById('paymentListMessage');
      if (!confirm(`Delete payment ${paymentNumber || ''}? This will reverse the invoice totals.`)) return;
    ...
    function renderTable(list = staffData) {
      ...
      if (count) count.textContent = `${list.length} staff member${list.length !== 1 ? 's' : ''}`;
      if (!list.length) {
    ...
    function filterStaff() {
      ...
      renderTable(staffData.filter(s =>
    ...
    document.getElementById('registerForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) { showMessage(msg, data.message || 'Registration failed', 'error'); return; }
    ...
    function onRegisterModalBgClick(e) {
      if (e.target && e.target.id === 'registerStaffModal') closeRegisterModal();
    }
    ...
    async function deleteStaff(id, name) {
      ...
      try {
        ...
        if (!res.ok) { showMessage(msg, data.message || 'Delete failed', 'error'); return; }
    ...
    function filterUsers() {
      ...
      renderUserTable(userData.filter(u =>
    ...
    function onUserModalBgClick(e) {
      if (e.target && e.target.id === 'registerUserModal') closeUserModal();
    }
    ...
    document.getElementById('createUserForm').addEventListener('submit', async (e) => {
      ...
      if (!name) { showMessage(msg, 'Name is required', 'error'); return; }
      if (!email) { showMessage(msg, 'Email is required', 'error'); return; }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showMessage(msg, 'Enter a valid email', 'error'); return; }
      if (!password || password.length < 6) { showMessage(msg, 'Password must be at least 6 characters', 'error'); return; }
      if (phone && phone.length > 30) { showMessage(msg, 'Phone is too long', 'error'); return; }
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) { showMessage(msg, data.message || 'Create user failed', 'error'); return; }
    ...
    async function saveName(e) {
      ...
      if (!name) { showMessage(msg, 'Name cannot be empty', 'error'); return; }
      try {
        ...
        const data = await res.json();
        if (!res.ok) { showMessage(msg, data.message || 'Update failed', 'error'); return; }
    ...
    async function savePassword(e) {
      ...
      if (!rules.every(r => r.ok)) {
        showMessage(msg, 'Password does not meet all requirements', 'error'); return;
      }
      if (newPw !== confPw) {
    ...
    function checkMatch() {
      ...
      if (!confPw) { lbl.textContent = ''; return; }
      if (newPw === confPw) {

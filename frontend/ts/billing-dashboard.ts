...
const staffHeaders = () => ({ 'Content-Type': 'application/json', 'x-staff-id': localStorage.getItem('staffId') });
    ...
    function init() {
      if (!isStaffLoggedIn()) { window.location.href = '/staff-login'; return; }
      loadSummary();
      loadInvoices();
      loadPayments();
      loadCredits();
    ...
    async function loadInvoices() {
      try {
        ...
        if (!invoices.length) { tbody.innerHTML = '<tr><td colspan="9" class="text-center">No invoices found</td></tr>'; return; }
    ...
    async function loadPayments() {
      try {
        ...
        if (!payments.length) { tbody.innerHTML = '<tr><td colspan="8" class="text-center">No payments</td></tr>'; return; }
    ...
    async function loadCredits() {
      try {
        ...
        if (!credits.length) { tbody.innerHTML = '<tr><td colspan="8" class="text-center">No credit records</td></tr>'; return; }
    ...
    async function loadOverdue() {
      try {
        ...
        if (!items.length) { tbody.innerHTML = '<tr><td colspan="7" class="text-center">No overdue invoices ðŸŽ‰</td></tr>'; return; }
        const now = new Date();
    ...
    async function generateInvoice() {
      ...
      if (!rentalId) { showMessage('Select a rental order', 'error'); return; }
      try {
    ...
    async function cancelInvoice(id) {
      ...
      try {
        ...
        const d = await r.json();
        if (r.ok) { showMessage('Invoice cancelled', 'success'); refresh(); }
        else { showError(d); }
      } catch (e) { showError(e); }
    }
    ...
    async function recalcLateFee(id) {
      try {
        ...
        const d = await r.json();
        if (r.ok) { showMessage('Late fee recalculated', 'success'); refresh(); }
        else { showError(d); }
      } catch (e) { showError(e); }
    }
    ...
    async function markCreditPaid(id) {
      ...
      try {
        ...
        const d = await r.json();
        if (r.ok) { showMessage('Credit marked as paid', 'success'); refresh(); }
        else { showError(d); }
      } catch (e) { showError(e); }
    }
    ...
    async function clearCredit(id) {
      if (!confirm('Clear this settled credit record?')) return;
      try {
        const r = await fetch('/api/billing/credits/' + id + '/clear', {
          method: 'PUT', headers: staffHeaders()
        });
        const d = await r.json();
        if (r.ok) { showMessage('Credit cleared', 'success'); refresh(); }
        else { showError(d); }
      } catch (e) { showError(e); }
    }
    ...
    async function saveSettings() {
      try {
        ...
        if (r.ok) showMessage('Settings saved!', 'success');
      } catch (e) { showError(e); }
    }

    async function archiveRecords() {
      if (!confirm('Archive all paid invoices older than the configured retention period?')) return;
      try {
        const r = await fetch('/api/billing/archive', { method: 'POST', headers: staffHeaders() });
        const d = await r.json();
        if (r.ok) { showMessage(d.message, 'success'); refresh(); }
        else { showError(d); }
      } catch (e) { showError(e); }
    }

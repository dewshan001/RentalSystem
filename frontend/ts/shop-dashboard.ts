...
let allItems = [];
    ...
    async function viewInvoice(orderId) {
      try {
        ...
        if (!response.ok) { showError(order); return; }
    ...
    async function closeOrder(orderId) {
      if (!confirm('Are you sure you want to close this order? This will calculate the final amount including any overdue charges.')) return;
